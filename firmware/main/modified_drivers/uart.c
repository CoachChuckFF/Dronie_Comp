...
//EDIT
#include "/Users/drkrueger/Work/Blizzard/SoC-It/Firmware_V4/main/include/blizzard_dmx_manager.h"
#include "/Users/drkrueger/Work/Blizzard/SoC-It/Firmware_V4/main/include/blizzard_rdm_manager.h"
#include "/Users/drkrueger/Work/Blizzard/SoC-It/Firmware_V4/main/include/blizzard_global_defines.h"
static volatile uint8_t TempDMX[MAX_DMX_SLOTS];
//END EDIT
...
#define UART_FULL_THRESH_DEFAULT        (8)
...
//EDIT
uint16_t currentSlot = CURRENT_SLOT_RESET;
uint8_t RDMState = 0;
volatile uint8_t* Dmx;
volatile RdmPacket* Rdm_Command;
...
void initDMXDriver(){
  Dmx = get_dmx();
  Rdm_Command = get_rdm_command();
  clearOutBuffers();
}

void resetDMXDriver(){
  currentSlot = CURRENT_SLOT_RESET;
  clearOutBuffers();
}

void rebootDMXDriver(){
    currentSlot = CURRENT_SLOT_RESET;
    gpio_set_level(DMX_SEL_PIN, 1);
    uart_disable_rx_intr(DMX_UART);
    uart_flush(DMX_UART);
}
//END EDIT
...
static void UART_ISR_ATTR uart_rx_intr_handler_default(void *param)
{
    ...
    uint8_t temp_dmx = 0;
    uint16_t max_slots;
    uint16_t buf_idx = 0; //really important this is uint16_t
    //END EDIT
    while(1) {
        ...
        if(uart_intr_status & UART_INTR_TXFIFO_EMPTY) {
            ...
            if(uart_num == DMX_UART) {
                ...
                if(currentSlot == CURRENT_SLOT_RESET) {
                    currentSlot = 0;
                }

                if(currentSlot == 0 && get_rdm_command_ready() && RDMState == 0){
                    prepareToLaunch(Rdm_Command);
                    RDMState = 1;
                }

                //add 2 to the length to send checksum
                max_slots = ((RDMState == 1) ? (Rdm_Command->_length + 2) : MAX_DMX_SLOTS);

                //incase RDM packet is malformed
                if(max_slots < currentSlot) max_slots = currentSlot;

                if((currentSlot + DMX_FIFO_SIZE) < max_slots){ //populate TXFIFO with DMX data

                    for(buf_idx = 0; buf_idx < DMX_FIFO_SIZE; buf_idx++) {
                        if(RDMState == 1){
                            WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), ((uint8_t*)Rdm_Command)[currentSlot + buf_idx] & 0xFF);
                        } else {
                            WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), Dmx[currentSlot + buf_idx] & 0xFF);
                        }
                    }
                    currentSlot += DMX_FIFO_SIZE;
                ...
                else { //send last byte & idle for a bit

                    //Send rest of bytes
                    for(buf_idx = 0; buf_idx < (max_slots - currentSlot); buf_idx++) {
                        if(RDMState == 1){
                            WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), ((uint8_t*)Rdm_Command)[currentSlot + buf_idx] & 0xFF);
                        } else {
                            WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), Dmx[currentSlot + buf_idx] & 0xFF);
                    ...
                    //UPDATED
                    uart_hal_tx_break(&(uart_context[uart_num].hal), (RDMState == 1) ? 256 : DMX_BREAK_SIZE);
                    ...
                    //set flags
                    if(RDMState == 1){
                        setRdmResponseReady(TRUE);
                        setRdmCommandReady(FALSE);
                        RDMState = 2;
                    }
            ...
            //EDIT RX
            if(uart_num == DMX_UART && !get_rdm_response_ready()){

                #ifdef TOUT_CHECK
                if(uart_intr_status & UART_RXFIFO_TOUT_INT_ST_M){
                    currentSlot = CURRENT_SLOT_RESET;

                    #ifdef NUKE_WITHOUT_CABLE
                        //nukes DMX array
                        for(buf_idx = 0; buf_idx < MAX_DMX_SLOTS; buf_idx++) {
                            TempDMX[buf_idx] = 0;
                            DMX[buf_idx] = 0;
                        }
                    #endif
                ...
                if((currentSlot + ((uint16_t)rx_fifo_len)) <= MAX_DMX_SLOTS){
                    while(buf_idx < rx_fifo_len) {
                        temp_dmx = uart_context[uart_num].hal.dev->fifo.rw_byte;
                        
                        // Dmx[currentSlot + buf_idx++] = temp_dmx;
                        
                        if(TempDMX[currentSlot + buf_idx] != temp_dmx){
                            TempDMX[currentSlot + buf_idx] = temp_dmx;
                            Dmx[currentSlot + buf_idx] = temp_dmx;
                        }
                ...
                currentSlot += rx_fifo_len;

                UART_ENTER_CRITICAL_ISR(&(uart_context[uart_num].spinlock));

                //UPDATED
                uart_hal_clr_intsts_mask(&(uart_context[uart_num].hal), UART_INTR_RXFIFO_TOUT);
                uart_hal_clr_intsts_mask(&(uart_context[uart_num].hal), UART_INTR_RXFIFO_FULL);
            ...
            //START EDIT - MAYBE REMOVE
            rx_fifo_len = uart_hal_get_rxfifo_len(&(uart_context[uart_num].hal));
            //END EDIT - MAYBE REMOVE
            ...
            if(uart_num == DMX_UART){
                ...
                //Picks up any straggler DMX signals
                if(currentSlot && currentSlot != CURRENT_SLOT_RESET){
                    rx_fifo_len = uart_hal_get_rxfifo_len(&(uart_context[uart_num].hal));
                    while(buf_idx < rx_fifo_len) {
                        if(currentSlot < MAX_DMX_SLOTS) {
                            Dmx[currentSlot++] = uart_context[uart_num].hal.dev->fifo.rw_byte;
                        }
                        buf_idx++;
                    }
                }

                //clears the rest of the buffer just in case
                for(buf_idx = currentSlot; buf_idx < MAX_DMX_SLOTS; buf_idx++){
                    Dmx[buf_idx] = 0;
                }

                currentSlot = 0;
                //allows the buffer to be self resetting
                uart_hal_rxfifo_rst(&(uart_context[uart_num].hal));
...
//EDIT RX - Will loop until all intr are clear
RX_END: ;;
//EDIT RX DONE
}
...
esp_err_t uart_driver_install(uart_port_t uart_num, int rx_buffer_size, int tx_buffer_size, int queue_size, QueueHandle_t *uart_queue, int intr_alloc_flags)
{
    ...
    //EDIT BOTH
    if(uart_num == DMX_UART){
        uart_hal_disable_intr_mask(&(uart_context[uart_num].hal), DMX_ALL_INTERRUPT_MASK);

        //Fuck ESP for not having these in the driver...
        // UART[uart_num]->conf1.rx_tout_en = 0;
        // #ifdef TOUT_CHECK
        //     UART[uart_num]->conf1.rx_tout_thrhd = ((DMX_TOUT_THRESH) & UART_RX_TOUT_THRHD_V);
        //     UART[uart_num]->conf1.rx_tout_en = 1;
        // #endif

        uart_hal_set_rxfifo_full_thr(&(uart_context[uart_num].hal), DMX_FULL_THRESH);
        uart_hal_set_txfifo_empty_thr(&(uart_context[uart_num].hal), DMX_EMPTY_THRESH);
    }
    //EDIT END
